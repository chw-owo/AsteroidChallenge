...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query(value = "select p from Post p inner join p.user inner join p.challenge inner join p.imageFile where p.challenge.id = :challengeId",
    countQuery = "select count(p) from Post p where p.challenge.id = :challengeId")
    ...
    @Query("select p from Post p join fetch p.challenge join fetch p.user join fetch p.imageFile where p.id = :postId")
